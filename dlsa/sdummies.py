...
def group_low_freq(sdf, dummy_columns, keep_top=.01, replace_with='other'):
    ...
    for string_col in dummy_columns:
        ...
        n_to_keep = sdf.groupby(string_col).agg((sparkcount(string_col)/total).alias('perc')).filter(col('perc')>keep_top).count()
        ...
        if n_to_keep != len(this_meta['ml_attr']['vals']):
            ...
            sdf = sdf.withColumn('ix_'+string_col,when(col('ix_'+string_col)>=n_to_keep,lit(n_to_keep)).otherwise(col('ix_'+string_col)))

        # add the new column with correct metadata, remove original.
        sdf = sdf.withColumn('ix_'+string_col, withMeta(col('ix_'+string_col), "", this_meta))
