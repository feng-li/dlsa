...
def updateR(xnew, xold, R = None, eps = np.finfo(np.float).eps):
    ...
    if(rpp <= eps):
        ...
        if(hasattr(R,"rank")): ### check if R is machine singular
            ...
            rank = np.NAN
        rank = rank + 1
    R = R = np.column_stack((np.row_stack((R, np.zeros(np.shape(R)[1]))), np.append(r,rpp)))
    setattr(R, "rank",rank)
    return(R)
...
def lars_lsa(Sigma0, b0, intercept = True, type = 'lar', eps=np.finfo(np.float).eps, max_steps = None):
	...
	while((k<max_steps)and(len(active)<m)):
		...
		if(not np.array(drops).any()):
			...
			C = C[abs(C) < Cmax - eps]
			for inew in new:
				R = updateR(Sigma[inew-1,inew-1], np.array(Sigma[inew-1, (active-1).astype(int)])[0], R, eps=eps)
